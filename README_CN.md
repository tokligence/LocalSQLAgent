...
### 🚀 多次尝试策略效果（模拟结果）
*数据来自 `tests/test_multi_attempt_effectiveness.py` 的概率模拟（8条合成查询），非真实LLM/数据库测得结果，生产环境不保证。*
...
**关键发现**：模拟结果显示多次尝试可从 46%→95%+ 提升；实际效果取决于模型、Schema 和数据。
